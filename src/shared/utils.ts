import { UrlVars } from "./types";

export function buildUrlSearchParams(urlVars: UrlVars): string {
  let retVal: string = "";
  const keys: string[] = Object.keys(urlVars);
  for (const key of keys) {
    retVal += `${key}=${urlVars[key]}&`;
  }
  retVal = retVal.substr(0, retVal.length - 1); // trim last `&` char
  return retVal;
}
